...
namespace Conduit.Business.Managers.EntityFramework
{
    public class EfFollowedUserManager : ManagerBase<FollowedPeople>, IFollowedUserServices
    {
        ...
        public async Task<FollowedUserListModel> GetListModel(int userid, int? limit , int? offset )
        {
            ...
            IQueryable<FollowedPeople> queryable = GetIncludes(p => p.TargetId); //Article Yapan User ve Taglar Gelsin
            ...
            var users = queryable.OrderByDescending(p => p.TargetId).Skip(offset.Value * limit.Value).Select(p=>p.TargetId).Take(limit.Value).ToList();


            await Task.Run(() => listModel.Users = _mapper.Map<List<User>, List<UserDto>>(null));
